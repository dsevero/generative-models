...
def get_model(input_shape, **kwargs):
    if kwargs["architecture"] == "DenseCNN":
        return _coconut_tail_call(Sequential, [Conv2D(filters=kwargs["conv2d_filters"], kernel_size=(5, 5), activation="relu", data_format="channels_first", input_shape=input_shape, padding="same"), MaxPooling2D(pool_size=(2, 2), padding="same"), Dropout(kwargs["dropout_rate"]), Conv2D(filters=kwargs["conv2d_filters"], kernel_size=(5, 5), activation="relu", data_format="channels_first", input_shape=input_shape, padding="same"), MaxPooling2D(pool_size=(2, 2), padding="same"), Dropout(kwargs["dropout_rate"]), Flatten(), Dense(kwargs["dense_units"], activation="relu"), Dropout(kwargs["dropout_rate"]), BatchNormalization(), Dense(10, activation="softmax"),])
    else:
...
def load_data(**kwargs):
    ...
    x_train, x_valid, x_test = reshape_channel_first(x_train / 255.0, x_valid / 255.0, test.values / 255.0)
    del data
    del test
...
# The data, split between train and test sets:
    ...
    history = model.fit_generator(datagen.flow(x_train, y_train, batch_size=kwargs["batch_size"]), validation_data=(x_valid, y_valid), steps_per_epoch=x_train.shape[0] // kwargs["batch_size"], epochs=kwargs["epochs"], verbose=1)
    ...
    with mlflow.start_run() as run:
        ...
        if kwargs["submit"] == "yes":
            ...
            submission_data = kaggle.api.competition_submit(file_name="MNIST-DenseCNN.csv", message=message, competition="digit-recognizer")
